...
public class DataUtil {
	 ...
	 * @throws SQLException 
	 */
	public static int getSqlCount(String sql,IDBSession session) throws SQLException{
		int count = 0;
		String [] str = sql.split("union");
		String sqlStr = "";
		for (int i = 0; i < str.length; i++) {
			if(i >= 1){
				if(!sqlStr.equals("")){
				...
				int x = 0,y = 0;  
				for (int j = 0; j <= sqlStr.length() - 1; j++) {   
					String getstr = sqlStr.substring(j, j + 1);   
					if (getstr.equals("(")) {
						x++;
					}
					if (getstr.equals(")")) {
				...
				if(x != y){
					...
					String [] s = sqlStr.split("from");
					if(null != s[0] && !"".equals(s[0])){
						sqlStr = sqlStr.replace(s[0], "select count(*) ").trim();
					}
					int x1 = 0,y1 = 0;  
					for (int j = 0; j <= sqlStr.length() - 1; j++) {   
						String getstr = sqlStr.substring(j, j + 1);   
